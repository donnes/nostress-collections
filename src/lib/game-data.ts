...
export const ARMOR_SETS = {
  ...
  sacred_fire: {
    name: "Sacred Fire",
    excludedPieces: ["boots"] as ItemPiece[],
  },
  ...
  thunder_hawk: {
    name: "Thunder Hawk",
    excludedPieces: ["helm"] as ItemPiece[],
  },
  hurricane: { name: "Hurricane", excludedPieces: ["helm"] as ItemPiece[] },
  piercing_grove: {
    name: "Piercing Grove",
    excludedPieces: ["gloves"] as ItemPiece[],
  },
...
export function isValidPieceForSet(
  setKey: ArmorSetKey,
  piece: ItemPiece
): boolean {
...
export function calculateSetCompletion(
  ...
  const completed = availablePieces.filter(
    (piece) =>
